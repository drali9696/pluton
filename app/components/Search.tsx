...
import { IconArrowRight, IconPlanet, IconSearch } from "@tabler/icons-react";
...
export const Search: FC<SearchProps> = ({
  ...
  const handleStream = async (sources: Source[]) => {
    try {
      // const prompt = endent`Наведите одговор од 2-3 реченице на упит на основу следећих извора. Будите оригинални, концизни, тачни и корисни. Цитирајте изворе као [1] или [2] или [3] након сваке реченице (не само на самом крају) да бисте подржали свој одговор (нпр. Тачно: [1], Тачно: [2][3], Нетачно: [ 1, 2]).
      // Provide a 2-3 sentence answer to the query based on the following sources.
      const prompt = endent`Provide a 3-4 sentence answer to the query based on the following sources. Be original, concise, accurate, and helpful. Cite sources as [1] or [2] or [3] after each sentence (not just the very end) to back up your answer (Ex: Correct: [1], Correct: [2][3], Incorrect: [1, 2]).
      
      ${sources
        .map((source, idx) => `Sources: [${idx + 1}]:\n${source.text}`)
        .join("\n\n")}
      `;
      ...
      while (!done) {
        ...
        done = chunkValue === "===DONE===";

        if (!done && chunkValue !== "===DONE===") {
    ...
    <>
      {loading ? (
        ...
        <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center space-y-6 px-3 pt-32 sm:pt-64">
          <div className="grid items-center justify-items-center align-center justify-center gap-4">
            <IconPlanet size={42} />
            <div className="ml-1 text-center text-4xl title">PLUTON</div>
          </div>
